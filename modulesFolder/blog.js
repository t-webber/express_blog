...
const blogPath = './dataFolder/blog/';
...
function createBlog(req, res, user) {
  ...
  if (!fs.existsSync(path)) {
    ...
    errorModule.error(res, user, '',
        'This blog already exists. Please consider changing the title.', 409);
  };
};
  ...
  return new Promise((resolve, reject) => {
    fs.readFile(blogPath + file, (err, data) => {
      ...
      listBlogs.push({id: file.slice(0, -4), title: elts[0],
        author: elts[1].slice(0, -4), body: data.toString()});
      resolve();
    });
  });
}
...
module.exports = {
  funlistBlogs, createBlog, editBlog, deleteBlog, getBlogContent,
};
